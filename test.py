...
# Step 3: Forecasting fill levels for the next 7 days and accuracy check
forecast_results = []
accuracy_results = []
...
for (bin_id, waste_type), bin_data in df.groupby(['bin_id', 'waste_type_name']):
    ...
    # Split data into training and testing sets (last 7 days for testing)
    train_data = bin_data[:-days_to_forecast]  # Use all but the last 7 days for training
    test_data = bin_data[-days_to_forecast:]   # Use the last 7 days for testing

    # Fit linear regression model on training data
    model = LinearRegression()
    model.fit(time_numeric[:-days_to_forecast], train_data['fill_level'])

    # Step 4: Forecast fill levels for the next 7 days
    ...
    predicted_fill_levels = model.predict(time_numeric[-days_to_forecast:]).flatten()
